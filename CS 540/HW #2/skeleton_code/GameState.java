...
public class GameState {
    ...
    public List<Integer> getMoves() {
        ...
        if (this.lastMove == -1) {
            ...
            int count = 2;
            while ((count * this.lastMove)<= this.size){
                if (stones[count * this.lastMove]){
                    move.add(count * this.lastMove);
                }
                count++;
            }
        }
    ...
    public double evaluate() {
        ...
        if (count % 2 == 0 && this.getSuccessors().size() == 0){
            return -1.0;
        }
        else if (count % 2 == 1 && this.getSuccessors().size() == 0){
            return 1.0;
        }
        else {
            if (count % 2 == 0){
