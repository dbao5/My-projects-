...
void access_data(mem_addr_t addr) {
   ...
   int successhit = 0;
   int lru = (*(cache + sbits) + 0) -> LRU;
   int most = (*(cache + sbits) + 0) -> LRU;;
   for(int i = 0; i < E; i++) {
      
   
      if (most < (*(cache + sbits) + i)->LRU) {
         most = (*(cache + sbits) + i)->LRU;
      }
      if (lru > (*(cache + sbits) + i)->LRU) {
         lru = (*(cache + sbits) + i)->LRU;
      }
   }

   for (int i = 0; i < E; i++) {
      if ((*(cache + sbits) + i)->tag == tbits){
         if ((*(cache + sbits) + i)->valid == '1') {
         (*(cache + sbits) + i)->LRU = most + 1;
         successhit = 1;
         hit_cnt++;
      }
   }
